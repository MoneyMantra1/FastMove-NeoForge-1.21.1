...
public class FastMove {
    ...
    public FastMove(IEventBus modEventBus, ModContainer modContainer) {
        ...
        NeoForge.EVENT_BUS.addListener(FastMove::onEntitySize);
    ...
    private static void onEntitySize(EntityEvent.Size event) {
        if (!(event.getEntity() instanceof Player player) || !(player instanceof IFastPlayer fastPlayer)) {
            return;
        }
        ...
        event.setNewSize(moveState.dimensions, true);
    }
